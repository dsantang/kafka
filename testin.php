...
$int16 = new Int16();
$int32 = new Int32();
$nonNullableString = new NonNullableString();
...
$schemas = [
    ...
    METADATA => [
        'version' => 7,
        'request' => new Schema(
            new Schema\Field('topics', new ArrayOf($nonNullableString, true)),
            new Schema\Field('allow_auto_topic_creation', $boolean)
        ),
        'response' => new Schema(
            $throttleTime,
            new Schema\Field(
                ...
                new ArrayOf(
                    new Schema(
                        new Schema\Field('node_id', $int32),
                        new Schema\Field('host', $nonNullableString),
                        new Schema\Field('port', $int32),
                        new Schema\Field('rack', $nullableString)
                    )
                )
            ),
            new Schema\Field('cluster_id', $nullableString),
            new Schema\Field('controller_id', $int32),
            new Schema\Field(
                'topic_metadata',
                new ArrayOf(
                    new Schema(
                        new Schema\Field('error_code', $int16),
                        new Schema\Field('topic', $nonNullableString),
                        new Schema\Field('is_internal', $boolean),
                        new Schema\Field(
                            'partition_metadata',
                            new ArrayOf(
                                new Schema(
                                    new Schema\Field('error_code', $int16),
                                    new Schema\Field('partition', $int32),
                                    new Schema\Field('leader', $int32),
                                    new Schema\Field('leader_epoch', $int32),
                                    new Schema\Field('replicas', new ArrayOf($int32)),
                                    new Schema\Field('isr', new ArrayOf($int32)),
                                    new Schema\Field('offline_replicas', new ArrayOf($int32))
                                )
                            )
                        )
                    )
...
$connector->connect('192.168.178.87:9092')->then(
...
//        $retrieveMetadata = $createRequest(++$currentId, METADATA, ['topics' => ['blah'], 'allow_auto_topic_creation' => true]);

        $connection->on('data', static function (string $chunk) use ($parseResponse): void {
...
//        $lengthMetadata = Message::allocate(4);
//        $lengthMetadata->writeInt($retrieveMetadata->length());

        $connection->write($lengthApiKeys->bytes());
        $connection->write($retrieveApiKeys->bytes());
//        $connection->write($lengthMetadata->bytes());
//        $connection->write($retrieveMetadata->bytes());
    }
)->otherwise(
